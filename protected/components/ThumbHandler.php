...
class ThumbHandler {
	 ...
	 *        	图片缩放后的高度
	 */
	public function resize($a, $b = null) {
		$num = func_num_args ();
		if (1 == $num) {
			$r = ( int ) $a;
			if ($r < 1) {
		...
		if ($this->_isFull ()) {
			switch ($this->mask_position) {
				...
				case 5:
					...
					$this->mask_pos_x = $this->mask_offset_x + $this->img_border_size/2;
					$this->mask_pos_y = $this->mask_offset_y + $this->img_border_size/2;
					break;
				default :
					// 默认将水印放到右下,偏移指定像素
					$this->mask_pos_x = $this->src_w - $this->mask_w - $this->mask_offset_x;
			...
			switch ($this->mask_position) {
				...
				case 4 :
					...
					//上面居中
				case 5:
					$this->mask_pos_x = $this->mask_offset_x + ($this->dst_w - $this->mask_w) / 2;
					$this->mask_pos_y = $this->mask_offset_y;
						break;
				default :
					// 默认将水印放到右下,偏移指定像素
					$this->mask_pos_x = $this->dst_w - $this->mask_w - $this->mask_offset_x - $this->img_border_size;
	 ...
	 * @return resource
	 */
	public function radius($corner) {
		if ($this->dst_w < $this->dst_h) {
			$imageSize = $this->dst_h;
		} else {
			$imageSize = $this->dst_w;
		}
	
		// $imageSize = 800;//$this->width;
		...
		imagecopymerge ( $image, $this->h_dst, 0, 0, 0, 0, $imageSize, $imageSize, 100 );
	
		// then just copy all the rounded corner images to the 4 corners
		// lt
		
		$lt_corner = imagecreatetruecolor ( $radius, $radius );
		$bgcolor = imagecolorallocate ( $lt_corner, 255, 255, 255 );
		...
		$corner_source = imagecreatefrompng ( $corner );
		$corner_width = imagesx ( $corner_source );
		$corner_height = imagesy ( $corner_source );
		imagecopymerge ( $corner_source, $image, 6, 6, 0, 0, $imageSize, $imageSize, 100 );
		imagecolortransparent ( $corner_source, $fgcolor );
		
		$this->h_dst = $corner_source;
		return $this;
	}
	/**
	 * 获取图片资源
